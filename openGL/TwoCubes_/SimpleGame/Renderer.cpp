...
void Renderer::Initialize(int windowSizeX, int windowSizeY)
{
	...
	//Load shaders
	m_SolidCubeShader = CompileShaders("./Shaders/SolidCube.vs", "./Shaders/SolidCube.fs");
	...
	//Initialize matrices
	m_v3CameraPos2D = glm::vec3(0.f, 0.f, 1000.f);
	m_v3CameraLookat2D = glm::vec3(0.f, 0.f, 0.f);
	m_v3CameraUp2D = glm::vec3(0.f, 1.f, 0.f);
	m_v3ModelRot2D = glm::vec3(0.f, 0.f, 0.f);
	m_v3ModelScale2D = glm::vec3(1.f, 1.f, 1.f);
	m_v3ModelTrans2D = glm::vec3(0.f, 0.f, 0.f);

	m_m4ProjView2D = glm::ortho(-windowSizeX / 2.f, +windowSizeX / 2.f, -windowSizeY / 2.f, +windowSizeY / 2.f, 0.f, 2000.f) * glm::lookAt(m_v3CameraPos2D, m_v3CameraLookat2D, m_v3CameraUp2D);
	m_m4Model2D = glm::translate(glm::mat4(1.0f), m_v3ModelTrans2D) * glm::eulerAngleXYZ(m_v3ModelRot2D.x, m_v3ModelRot2D.y, m_v3ModelRot2D.z) * glm::scale(glm::mat4(1.f), m_v3ModelScale2D);
}
...
void Renderer::CreateVertexBufferObjects()
{
		...
		=
	{
		-temp, -temp, 1.f,
		-temp, temp, 1.f,
		temp, temp, 1.f, //Triangle1
		-temp, -temp, 1.f,
		temp, temp, 1.f,
		temp, -temp, 1.f, //Triangle2
	};
...
void Renderer::DrawSolidCube(float x, float y, float z, float eulerX, float eulerY, float eulerZ, float size)
{
	GLuint shader = m_SolidCubeShader;
	ka += 0.1;
	//Program select
	glUseProgram(shader);
	...
	GLuint rot = glGetUniformLocation(shader, "u_Rot");

	glm::mat4 m4Model = glm::translate(glm::mat4(1.0f), glm::vec3(x, y, z)) * glm::eulerAngleXYZ(eulerX, eulerY, eulerZ) * glm::scale(glm::mat4(1.f), glm::vec3(size, size, size));
	glm::mat4 m4Rot = glm::eulerAngleXYZ(eulerX, eulerY, eulerZ);

	glUniformMatrix4fv(projView, 1, GL_FALSE, &m_m4ProjView2D[0][0]);
	glUniformMatrix4fv(model, 1, GL_FALSE, &m4Model[0][0]);
	glUniformMatrix4fv(rot, 1, GL_FALSE, &m4Rot[0][0]);
...
void Renderer::DrawSolidRect(float x, float y, float z, float size, float r, float g, float b, float a, float level)
{
	...
	m_m4Model2D = glm::translate(glm::mat4(1.0f), glm::vec3(x, y, z)) * glm::eulerAngleXYZ(m_v3ModelRot2D.x, m_v3ModelRot2D.y, m_v3ModelRot2D.z) * glm::scale(glm::mat4(1.f), glm::vec3(size, size, 1.f));

	glUniformMatrix4fv(projView, 1, GL_FALSE, &m_m4ProjView2D[0][0]);
	glUniformMatrix4fv(model, 1, GL_FALSE, &m_m4Model2D[0][0]);
